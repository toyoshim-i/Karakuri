...
- **`pre-commit`**: Runs `cargo fmt --check` against staged Rust files to enforce workspace formatting standards, strictly rejects any file exceeding 1,000 lines, enforces workspace-wide Clippy cleanliness (`cargo clippy --workspace --all-targets -- -D warnings`), and warns if a modified file exceeds 800 lines for the first time.
