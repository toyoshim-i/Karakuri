...
| P-0054 | A gate that takes minutes stops being a gate | [P-0089](../principles/0089-a-check-you-have-not-watched-fail-is-guessing.md) | Its checking half is the new rule's — *a gate's test is something that must not pass*, which this one first failed by passing, because `rustfmt --check` reading stdin prints its diff and exits 0. [ADR-0109](0109-format-the-workspace-and-split-the-gate.md) carries that and the original split; [ADR-0114](0114-tests-run-when-somebody-asks-not-when-git-does.md) carries the move to a tag push. **Its scheduling half is stated again by [P-0057](../principles/0057-run-what-the-question-needs-when-it-is-asked.md)**, which names the same hooks, the same document and ADR-0114, and which no rule in this batch retires |
