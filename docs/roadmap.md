...
2. **Output Plugin Sinks & Decoupled Discovery Architecture** *(In Progress — Syphon completed on macOS, Spout in progress on Windows)*:
   ...
   - **Platform Plugin Binaries**: macOS `karakuri-syphon` (completed); Windows `karakuri-spout` (Spout2 named shared memory sender over `dxgi` handle in standalone repo `Karakuri-spout`); future NDI. Spout/NDI pending beside the window, as [plugins.md](plugins.md) specifies ([ADR-0358](adr/0358-the-projector-is-fullscreened-by-the-operating-system-on-the-display-it-is-on-and-another-application-is-reached-through-a-plugin.md)).
