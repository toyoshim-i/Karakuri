...
3. **Alphabetically Sorted CLI Selection Menu & Executable Detection**:
   ...
   - Dropdown presenting sorted agent presets with `custom...` pinned at the end:
     ...
     6. `deepseek` (DeepSeek CLI)
     7. `grok` (xAI Grok CLI)
     8. `kimi` (Moonshot Kimi CLI)
     9. `mimo` (Xiaomi MiMo CLI)
     10. `minimax` (MiniMax CLI)
     11. `mistral` (Mistral AI CLI)
     12. `muse` (Muse CLI)
     13. `ollama` (Local LLM runner for offline live venues)
     14. `pi` (Pi CLI)
     15. `qwen` (Alibaba Qwen CLI)
     16. `custom...` (Arbitrary user command and arguments)
