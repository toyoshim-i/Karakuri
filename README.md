...
than the flag's number — and the parameters, bindings, camera, layering, node names and edges
the run was given. `--load-set` reads it back, and a run driven by the file renders
...
| [docs/adr/](docs/adr/) | Every architecture decision, with the alternative that lost and why. Append-only; start at [ADR-0000](docs/adr/0000-record-decisions-here-and-standing-rules-in-principles.md) |
| [docs/principles/](docs/principles/) | The rules currently in force, one per file. `ls` is the index, because each filename is the rule |
