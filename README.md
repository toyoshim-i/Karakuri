...
| Examples | Seven procedures. `drift_shell + soft_points` is the default and is particles; **`drift_shell + drift_streaks` is the same L1 drawn as segments**, which is what a line renderer costs — one L4, no change to the geometry. **`strand_shell + strand_strokes` is line art made *without* a line primitive** — `seed` split into a strand id and a position along it, points laid densely along an analytic curve, hue and width taken from the strand because per-element colour makes a line read as noise. It is kept as written now that `lines` exists, because it is the measurement of what one topology cost: roughly twice the elements, and a technique nobody finds unforced. `beat_shell` reads `beats`, `spark_fountain` spawns and kills |
| **Rendering primitives** | **Two topologies, one blend.** `Topology` has `Points` and `Lines`; `Blend` still has only `Additive`. **`lines` landed as M3's first item**, and it cost the engine nothing: it is the same quad, six vertices per instance and a `TriangleList` pipeline, laid along the segment from `clip` to `clip_b` instead of around a point. An L4 says it draws segments by assigning that second endpoint and nothing else — there is no `topology` on an L4 header, because a second endpoint is the only thing it could mean and a header field would be a place for the file to contradict itself. The two ends of a segment both belong to **one element**: no element links to another, deliberately, since an index into the element buffer is what compaction invalidates, so a stroke survives spawning and killing material where a shared vertex would not. **The vocabulary is still ahead of the engine**: `sd_sphere`, `sd_box`, `sd_torus`, `sd_plane`, `op_union`, `op_smooth_union`, `op_subtract` and `op_intersect` are accepted by the checker, used by no example, and have no renderer that could consume them — an SDF is a shape you march, and nothing marches. A fullscreen raymarch and `blend weighted` are what remains |
