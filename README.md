...
chain — an L2, an L3, a field, or a second geometry — is refused rather than saved short:
the format names an L1 and its renderers, and it has no way to name the rest.
...
| What a Set file cannot carry | Named rather than dropped, and printed on load. A `param` may be a vector and the engine's map holds `f32`; a `camera` record carries two of `Orbit`'s six fields, so the other four come back as defaults; and a slot holding a chain or a second geometry is **refused outright**, because the format names an L1 and its renderers and cannot name the rest. Each is a real disagreement between the format and the engine rather than an omission in the loader, and a Set file that half-applied in silence is the failure this repository keeps refusing. The one that used to head this list — `seed`, `capacity` and `param` keyed by layer against an engine holding one of each per Set — is gone: params are per node, the salt is per source, and each source runs at its own capacity |
...
| `kind Field` | Works. A signed distance function in a file of its own: handed a `point`, it assigns a `distance`, and it draws nothing and holds no elements. **It has a file and no node** — the L5 argument run backwards, since a `kind` says what a procedure lowers to and this has only code to lower — so it is spliced as one WGSL function into whichever procedures call `field(p)`, and into no others. One per Set, on the camera's terms. Its cost is **per evaluation** and the caller pays it once per call, so a marcher and a field that each fit alone can still be refused together, with both figures in the message. Out: several per Set, which needs naming, and naming is the notation fan-in brings |
