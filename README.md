...
a Set file referencing them by hash, with the capacity, parameters, bindings, camera, node
names, edges and seed the run was given. `--load-set` reads it back, and a run driven by the file renders
...
hot-swap rebuild and the MCP surface all reach past a slot's first geometry. What is still
capped at one apiece is a field and a camera, which want the same notation carried one layer
further.
