...
| Diagnostics | Works, and it turned out to have a second audience. Fifty-eight diagnostics carry a `hint`, written to be kind to whoever typed the mistake. The first real MCP session showed they are also **the specification an LLM reads**: denied an example to copy, a model probed the checker instead — safely, because `write_procedure` checks before it writes, so a failed compile changes nothing. `hint: v0.2 defines 'points' only` closed off line primitives in one try. And the hint that carried a *reason* — `loop bounds cannot reference a param… cost estimation needs them fixed at parse time` — changed the model's whole approach rather than its syntax: it concluded that integrating a streamline per element was incompatible with the cost model and switched to an analytic curve. **A hint that only corrects syntax produces a procedure that compiles and cannot be afforded.** Every hint should say why |
| Examples | Five procedures, in two pairs and a spare. `drift_shell + soft_points` is the default and is particles; **`strand_shell + strand_strokes` is line art made without a line primitive** — `seed` split into a strand id and a position along it, points laid densely along an analytic curve, hue and width taken from the strand because per-element colour makes a line read as noise. It exists because a model was asked for lines and found this; keeping it is what stops the next reader assuming points mean particles. `beat_shell` reads `beats`, `spark_fountain` spawns and kills |
| **Rendering primitives** | **One.** `Topology` has a single value, `Points`, and `Blend` a single value, `Additive` — both declared as enums so that a second is an addition rather than a format change, and neither has ever had one. Everything this system has ever drawn is an additive point sprite, which is why every example looks like a relative of every other. **The vocabulary is already ahead of the engine**: `sd_sphere`, `sd_box`, `sd_torus`, `sd_plane`, `op_union`, `op_smooth_union`, `op_subtract` and `op_intersect` are accepted by the checker, used by no example, and have no renderer that could consume them — an SDF is a shape you march, and nothing marches. This is the sharpest limit on what the whole project can express, and it is the first thing M3 should spend its budget on. **How hard it bites was measured by accident**: asked for line art, a model decomposed `seed` into a strand id and a position along a strand, laid points densely along an analytic curve, and moved hue and width from the element to the strand — per-element hue would make each sample its own colour and the line would read as noise. It works, and it cost half the element budget and a technique nobody finds unforced. The limit is not "one look is possible", it is "every other look is paid for twice" |
