...
/// One live region's declaration: what one update of it costs, and how stale it
/// may get.
///
/// The unit is a region and never a slice of time, which is ADR-0210 stated as
/// a type: a region is redrawn whole or not at all, so what declares is a node
/// of the arrangement rather than an animation, a control or a frame.
...
/// its two faders share a period, a curve and a staleness, so a parked slot and
/// eight armed fades declare once
...
pub struct Declared {
    ...
    pub cost: Duration,
    /// How stale this region may get before what it shows misrepresents what the
    /// instrument is doing, in wall time and never in frames — a tolerance stated
    /// in frames doubles when the rate halves, which is to say it becomes most
    /// permissive exactly when the machine is most loaded.
    ///
    /// It is a constant of the presentation and the frame does not move it, which
    /// is what lets `tests/schedulable.rs` sum it: a staleness that fell with the
    /// tempo would make `Σ (cost / staleness)` a function of how fast the music is
    /// ([ADR-0212](../../../docs/adr/0212-the-beat-is-a-light-that-travels-and-it-declares-for-itself.md)).
...
/// bay* is not an operation this panel has — what repaints is the panel and not
/// the chip, which
/// [ADR-0188](../../../docs/adr/0188-a-pending-transition-says-it-is-pending-and-no-surface-holds-the-rule.md)
/// and
/// [ADR-0190](../../../docs/adr/0190-the-parked-tally-rolls-because-two-lamps-do-not-fit-in-fifty-three-pixels.md)
/// both state as the honest cost of the roll. So the price of servicing any one
/// region's deadline is the price of the whole immediate-mode pass, and a
/// per-region figure would be a number describing work this console cannot do.
///
...
/// declares what it needs; what the panel can afford is decided elsewhere … a
/// deliberate over-declaration that a scheduler can refine downwards."* It
...
/// the second condition — and that day this constant becomes a per-region
/// number and `Declared::cost` stops being written from one place.
///
...
/// read."* Nothing reads this at runtime and nothing samples a clock to get it.
///
/// 1.26 ms, taken on 2026-08-28 over five runs of `crates/karakuri/src/main.rs`
/// on an Apple M4 Pro at 1440x900 logical, host clock, debug profile with
/// dependencies at opt-level 3, with nothing touching the window. It is `ui +
/// textures + buffers + record` — the whole immediate-mode pass, plus the
/// panel's own texture and geometry uploads and the recording of its render
/// pass. The five read per-frame medians of 1.047, 1.224, 1.259, 1.281 and
/// 1.340 ms, and this is the middle of them. Read it as an order of magnitude:
/// the same machine reports about a sixth of these numbers with its other cores
/// loaded, which is why what holds this figure honest allows a factor of two
/// either way rather than a digit.
///
/// Three numbers on that frame are deliberately not in it. The engine pass,
/// because the picture is the governor's and counting it here would count it
/// twice (ADR-0210). The vsync wait, because a frame blocked in
/// `get_current_texture` is not paying for anything. And the submission — about
/// 2.4 ms, larger than this whole figure — because it carries the engine's half
/// of the frame as well and is paid whenever anything is submitted, so charging
/// it to a region would charge a region for a frame it did not ask for.
///
...
/// already in that file for `WRITTEN_ALLOCS` and the reason that one exists: a
/// number written into prose reads as current forever, and this repository has
/// watched exactly that happen. It reports rather than fails, and that is a
/// decision rather than a lapse: this machine reports about a sixth of these
/// numbers with its other cores loaded, so an assertion on a millisecond would
/// be a gate nobody could keep passing, and *flaky is worse than broken*.
///
/// What is asserted, without a window or a clock, is the other half:
/// `tests/schedulable.rs` holds every declaration's cost to this one constant,
/// so a per-region figure cannot be invented while the panel is still redrawn
/// whole.
pub const PANEL_PASS: Duration = Duration::from_micros(1260);

/// What the panel may spend on a frame, and the divisor in ADR-0164's second
/// condition.
///
/// It is the console's only written-down budget and it is the whole frame's,
/// which is worth reading twice. `view::Transport::budget_ms` is *"what a frame
/// has to fit in"* — the `/16.6` the transport row draws — and
/// `crates/karakuri/src/main.rs` fills it from the display's refresh interval,
/// because the surface is `PresentMode::Fifo` and a frame longer than one
/// interval is a frame that misses a vsync. Nothing anywhere writes down a
/// panel's share of that. So the conditions are asserted in their most
/// permissive form — the panel is allowed the whole frame — and what that costs
/// is that they cannot catch a panel which fits the frame while leaving the
/// engine nothing. Naming a share is a decision, and it belongs with whoever
/// takes it rather than with the test that noticed it was missing.
///
/// Fixed at the 60 Hz interval rather than read from the display, which is
/// ADR-0164 in one number: *"the panel's absolute budget does not grow when the
/// frame lengthens. The extra time belongs to the engine, which is what is
/// actually struggling."* So this stays put while [`FRAME_INTERVAL`] moves, and
/// the panel's share of a lengthened frame shrinks rather than growing — which
/// is the whole point of the two being separate constants that happen to be
/// equal today.
...
/// rate `crates/karakuri/src/main.rs` reads off the monitor it opens on, and
/// the rate every figure in this repository is taken at.
///
/// The same number as [`BUDGET`] and not the same quantity. This one is what
/// the window is actually held to and it moves — a 120 Hz display halves it,
/// and `PresentMode::Fifo` quantising a missed frame doubles it. The budget
/// does not move with it. `Σ (cost / staleness) ≤ budget / frame interval` is
/// therefore a *fraction of each frame the panel may have*, and it gets smaller
/// as the frame gets longer.
...
///
/// Marked as `docs/contributing.md` §4 asks. Forced: the fraction is well under
/// a half, because *most of the budget* is the failure the clause names and a
/// half is not distinguishable from it. Preference: which fraction under a
/// half.
///
/// A quarter, on two readings that bracket it. A tenth is 1.67 ms against a
/// panel pass measured at 1.26, whose five runs already spread 1.05 to 1.34 — a
/// rule that passes by a third of what the measurement itself moves by is a
...
/// `crates/karakuri/src/main.rs` allows its own quoted figure and for the same
/// reason.
///
/// A second region declares now and this number did not have to move, because
/// `max(cost)` is still one number: both of them declare one whole
/// [`PANEL_PASS`], which is what immediate mode makes true. It is the day a
/// region declares a cost of its own that `max` starts choosing, and that is
/// the day to revisit this fraction.
