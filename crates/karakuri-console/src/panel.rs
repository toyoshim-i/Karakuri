...
/// at all: a 9px gap is not a target a hand finds.
pub const GRAB: f32 = 6.0;

/// How far short of the ask a boundary has to land before a stop is what put it
/// there, in pixels.
///
...
/// chosen to reproduce one — and reads the position back out of the next solve,
/// so a landing is a few ten-thousandths of a pixel off its ask by arithmetic
/// alone. This is the line between that and a constraint, and there is nothing
/// between the two to be careful about: it is two orders of magnitude above
/// single-precision noise at these coordinates and an order below anything an
/// eye or a display can resolve, so a `held` is a stop and nothing else.
const STOPPED: f32 = 0.05;

/// How far a drag has to go on past a stop before it is pulling a region out
/// through its own edge, rather than leaning on a boundary that will not move,
/// in pixels.
///
...
/// which it could have taken hold of it at all — so it is no longer pressing on
/// the boundary, it is pulling the region out from behind it. A second number
/// would be a second answer to a question the panel has already answered, which
/// is
...
///
/// A drag reports what the layout did, not what the pointer asked, and at sixty
/// asks a second the difference is a readout a person can follow against one
/// they cannot. Half a pixel is the smallest move that can change what is drawn
/// — below it the boundary is on the same physical pixel it was — so this
/// reports every move a viewer could see and none that they could not. A stop
/// starting or stopping to hold the drag is reported whatever the distance:
/// that is a change in *what is happening*, not in where the boundary is.
const WORTH_SAYING: f32 = 0.5;

/// One node of the arrangement, flattened in tree order.
///
/// Two facts, and both of them are about the flattening rather than about the
/// node. Everything else — whether it is a view, what encloses it, where it
/// solved to — is [`Layout`]'s to answer and is asked of it by `id`. This
...
struct Boundary {
    ...
    /// Subtracted from every later coordinate so the boundary does not jump to the
    /// pointer on the first move.
    offset: f32,
    /// Where the boundary was when this drag last reported anything, and whether
    /// what it reported was that a stop was holding it.
    ///
    /// A drag is reported when the boundary moves, not when the pointer does. A
    /// pointer dragged on past a stop asks for a new position sixty times a second
    /// and the boundary does not move for any of them; a report per ask is a flood
    /// that says the same thing every time, and it is worst exactly where a person
    /// is looking hardest. So a stop is reported once, and the next [`Dragged`] is
    /// the one where the boundary moves again.
    said: Option<f32>,
    held: bool,
    ...
    /// other's inverse the instant either fires — a pane closed at `GRAB` past its
    /// stop puts its own edge under a pointer that is `GRAB` the other side of it,
    /// which is the open condition, met on the very next move — and the pane would
    /// flicker at sixty asks a second for as long as the button was down.
    ///
    /// So it is once per gesture, and the way back is another gesture. That is
    /// [`Fading::said`]'s rule about a drag that has already said what it had to
    /// say, on a thing that happens once rather than on a value.
    acted: bool,
}
...
struct Fading {
    grab: Grab,
    /// What this gesture last emitted, or `None` before it has emitted anything.
    ///
    /// It is what was *said*, not what the deck is at, exactly as
    /// [`Boundary::said`] is. The difference matters and is the whole of what keeps
    /// this crate off the engine's state: nothing reads this to draw anything, and
    /// nothing hands it back out. It exists so that a pointer dragged past the end
    /// of a track — which asks for 1.0 sixty times a second and moves nothing —
    /// emits one operation and then none, which is what lets [`crate::repaint`]
    /// answer that a drag which changed no value is owed no frame. The seed is the
    /// same rule at the other end of the gesture: a press that moves nothing emits
    /// nothing.
    ///
    /// A boundary needs [`WORTH_SAYING`] here and a fader does not: half a pixel is
    /// a threshold on a *position*, and this is an exact comparison of the value
    /// that would be sent. Every value that differs is a real change to the mix,
    /// however small.
    said: Option<f32>,
}
...
struct Carrying {
    ...
    set: String,
    /// Whether that name is a procedure's, which decides which load the release
    /// names: `Operation::LoadProcedure` writes one layer over what the deck is
    /// playing where `Operation::LoadSet` replaces every layer (ADR-0338).
    ///
    /// It is carried and not looked up. The operation is built at the release and
    /// the listing the row came from is not in hand there — a listing the host
    /// rewrote mid-drag would answer about a different row — so what a press
    /// resolved travels with the name it resolved.
    procedure: bool,
}
