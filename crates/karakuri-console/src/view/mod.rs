...
mod tests {
    ...
    //!
    //! **It is now entirely the Inspector's.** [`pane_box`] and [`group_h`]
    //! are the pane's arithmetic, and the public [`inspector`] is that
    //! arithmetic plus a layout lookup. The one figure the transport row
    //! wrote here — `offset_text` — moved with the rest of that bay to
    //! [`transport`] under
    //! [ADR-0121](../../../../docs/adr/0121-moving-code-leaves-its-reasoning-behind.md),
    //! and its own assertion moved with it into `transport`'s own `tests`
    //! module, on this module's precedent rather than as an exception to it.
    //!
