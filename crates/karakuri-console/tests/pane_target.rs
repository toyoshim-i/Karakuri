//! **The pulldown on an Inspector pane's head**: *point this pane at another
//! deck* —
...
//!    measure every other `▾` on this console is drawn at — and that the run
//!    now stops short of it rather than over it.
...
//! 5. That a deck the mixer draws no strip for is refused rather than clamped.
...
//! press on the mark and every press while the card is down, and that the
//! window re-reads the pane the pick moved. Those are `input::PROBES`' row,
//! `claim`'s rule 2 and `crates/karakuri/src/main.rs`'s `pointed_pane`.
...
/// A console with `decks` strips and two panes on the first two of them,
/// which is what a run opens with.
...
/// The pulldown in pane `index`'s head, as the paint and the press both ask
/// for it.
fn pulldown(
    panel: &Panel,
    ctx: &egui::Context,
...
/// `deck_name`'s guard read through: the mark hangs off the run and there is
/// no run until a pass has laid one out.
#[test]
...
/// **The card is shut until a press opens it**, and a shut card has no rows —
/// which is what stops `PaneTarget::row` handing out a rectangle for a list
/// nobody opened.
#[test]
...
/// **A pick moves that pane and no other pointer**: not the deck selection,
/// not the pane next door, not the Library bay's load target.
#[test]
...
/// **Two panes are two answers**, which is what makes slots C and D reachable
/// at all: a run with four decks can show any two of them.
#[test]
...
/// **A deck the mixer draws no strip for is refused rather than clamped**,
/// which is `View::select`'s and `View::aim_at`'s rule read a third time: a
/// pick of deck D on a two-slot deck means *deck D*, and clamping would point
/// the pane at deck B.
#[test]
...
/// **A pane index past the panes there are is refused too**, and it is a
/// caller's error rather than a state.
#[test]
