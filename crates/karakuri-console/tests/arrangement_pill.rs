...
fn the_pill_is_the_rows_own_geometry() {
    ...
    let arr = filed();
    let pill = arrangement(&ctx, panel.layout(), Some(mock()), &arr).expect("the row draws it");
...
fn the_capsule_is_as_wide_as_the_name_in_it() {
    ...
    let of = |arr: &Arrangement| {
        arrangement(&ctx, panel.layout(), Some(mock()), arr)
            .expect("a pill")
            .pill
            .width()
    };
...
fn the_pill_clears_every_boundarys_grab() {
    for viewport in [SMALLEST, PLAUSIBLE] {
        ...
        let arr = filed();
        let pill = arrangement(&ctx, panel.layout(), Some(mock()), &arr).expect("a pill");
        let strip = rect_of(panel.layout(), "transport");
...
fn a_control_that_has_not_been_drawn_is_not_there() {
    ...
    assert_eq!(
        arrangement(&fresh, panel.layout(), Some(mock()), &filed()),
        None
    );
    ...
    // second one.
    let ctx = drawn_once();
    assert_eq!(arrangement(&ctx, panel.layout(), None, &filed()), None);
}
...
fn a_press_on_the_pill_opens_the_menu_and_a_press_again_shuts_it() {
    let (panel, ctx) = console(PLAUSIBLE);
    let mut arr = filed();
    let shut = arrangement(&ctx, panel.layout(), Some(mock()), &arr).expect("a pill");
    assert_eq!(
        shut.menu, None,
    ...
    arr.opened();
    let open = arrangement(&ctx, panel.layout(), Some(mock()), &arr).expect("a pill");
...
fn the_menu_lists_the_names_handed_in_and_a_pick_puts_that_one_back() {
    ...
    none.opened();
    let empty = arrangement(&ctx, panel.layout(), Some(mock()), &none).expect("a pill");
...
fn save_means_the_name_in_use_and_asks_for_one_where_there_is_none() {
    ...
    in_use.opened();
    let pill = arrangement(&ctx, panel.layout(), Some(mock()), &in_use).expect("a pill");
    let row = pill.row(0);
    ...
    fresh.opened();
    let pill = arrangement(&ctx, panel.layout(), Some(mock()), &fresh).expect("a pill");
...
fn the_pill_is_the_harnesss_and_is_stored_nowhere() {
    ...
    let first = arrangement(&ctx, panel.layout(), Some(mock()), &one).expect("a pill");
    let other = arrangement(&ctx, panel.layout(), Some(mock()), &two).expect("a pill");
    assert_ne!(
        first, other,
    ...
    assert_eq!(
        arrangement(&ctx, panel.layout(), Some(mock()), &one).expect("a pill"),
        first,
...
fn a_folded_or_soloed_or_narrow_row_draws_no_pill() {
    ...
    panel.solve();
    let arr = filed();
    assert!(arrangement(&ctx, panel.layout(), Some(mock()), &arr).is_some());
