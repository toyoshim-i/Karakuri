...
//!
//! 1. That a console with no store behind it draws nothing there — not an empty
//! list and not a `0 of 0` under one, either of which is a reading of a library
//! nobody opened. It is asserted by drawing a frame and counting what landed in
//! the bay, because *nothing is drawn* is a claim about the paint pass and not
//! about a rectangle. 2. That the foot's two numbers are the two the mock's are
//! — how many rows are listed against how many the store holds — which is the
//! whole of what the bay says about a library taller than its list. 3. Where
//! the rows and the foot are, derived from the bay's own geometry and the
//! mock's boxes. 4. That nothing in it is a control, stated rather than
//! inferred from the absence of a hit test: `claim` gives every point of this
//! bay to `egui` unless a boundary has it. 5. That the names are the harness's
//! and the console keeps no copy.
//!
...
//! this bay is the full width of the list, so no rectangle here is the width of
//! the type in it. The two tests that draw a frame need a context all the same,
//! because painting type is what they count.
...
/// A listing of Sets and nothing else, which is what every test in this file
/// that says nothing about kinds is about: a row with no entry in the kinds
/// beside it is a Set with no badge, which is what this bay drew before
...
/// The mock's own library, as names: five Sets, in the order it draws them.
///
...
/// scopes is a console nobody has told what libraries there are — which is what
/// `View::new` starts at, and what the two tests that draw a frame are about.
...
/// What the foot's load control is aimed at, everywhere in this file that is
/// not about the pulldown: deck A, no strips behind it and the list shut, which
/// is what [`View::new`] answers and what every console here that has not been
/// handed a mixer is.
...
/// conclude. So a shape counts where it sits in the row down the column and
/// reaches it across: the bay's card is the shape that rules out, since it is
/// the height of the whole bay.
...
/// The viewport the pulldown's list is held inside, as `egui`'s rectangle — the
/// same one `View::draw` hands it.
...
/// A strip, as far as this bay cares: something for a deck to be selected on.
/// Every reading in it is beside the point here — `mixer.rs` is where a strip's
/// values are asserted — and what matters is that there are as many of them as
/// there are decks to name.
...
///
/// The ranges are the mock's own strings, because that is what crosses the seam
/// — `view::Published::range` is what the host spelled and this crate draws
/// (ADR-0156). What they are spelled *from* is a card's `param` record, and
/// `karakuri/src/main.rs` is where that spelling lives.
fn reading() -> Reading {
    Reading {
        id: "drift_night".to_owned(),
        knobs: [
