...
/// A view with a deck behind it — two panes — which is what `View::draw` paints
/// from and what a press is resolved against, one value.
...
/// Where the `index`th row of the `group`th node goes, worked out here from the
/// stylesheet's own numbers rather than asked of the crate — so that a row
/// drawn in the wrong place is two derivations disagreeing rather than one
...
/// Where the knob of that row sits, from the row's own rectangle and the value
/// the row is carrying — `.param`'s four tracks, with the fader taking what is
/// left between the name and the figure.
...
/// Every shape a whole frame put wholly inside `rect`, as bounds — `fader.rs`'s
/// helper, and here for its reason: what a value moves is where a mark is, and
/// a failure that prints rectangles is one a reader can act on.
fn drawn(
...
/// `.param`'s own height from there — and the last row of a group ends exactly
/// where the group does, which is `group_h` and the row's offset being one
/// statement.
#[test]
...
/// Nothing else in the pane is a control of this derivation's — the two heads
/// above the body, a node head, the renderer row, and the ordinal, the name and
/// the figure of a row.
#[test]
...
/// Every row is its own control, across both groups and across the renderer
/// row's offset — five rows, one of which is not a handle.
#[test]
...
/// Each pane names the deck it is pointed at, which is `Pane::deck` and not the
/// deck selection.
#[test]
...
/// The knob is where the value put it, so the same row at two values is two
/// knobs — and a press at one of them does not find the other.
#[test]
