//! Transport row `audio-in` pill: device awareness, placement, boundary grab
//! clearance, modal card handling, and source attachment operations.
...
/// An unconfigured console renders no pill, while a console informed of no inputs renders `none` (ADR-0156).
#[test]
...
/// The pill clears boundary grabs vertically by 9.75px (15.75px clearance vs 6px grab).
#[test]
...
/// Selecting a device row emits `AttachBeatSource` with the exact device name.
#[test]
...
/// An open audio-in card intercepts pointer events across boundaries until closed.
#[test]
...
/// When no inputs exist, an informative notice is displayed without clickable rows (P-0094).
#[test]
