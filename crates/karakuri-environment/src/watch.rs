...
//!
//! One of these per deck slot, over that slot's own files. A slot is the unit
//! that gets replaced — that is what it means for each slot to own its own
...
//! ## Polling, not `notify`
//!
//! The worker is a poll loop already: it has to wake regularly to free Sets the
//! render thread retired, so there is no blocking `recv` for a filesystem event
//! to replace. Given that, `notify` would add a dependency, a platform backend
//! per OS, and a second event vocabulary to debounce, in exchange for shaving a
//! tenth of a second off a loop whose other end is a human editing a file.
//! Reading a handful of small files every hundred milliseconds is not a cost
//! worth avoiding here, and it is the same amount of code.
//!
...
//! truncated for a moment, and reading it then produces a parse error against a
//! file that is about to be fine. So a change is not acted on the poll it is
...
//!
//! It answered `None` until 2026-09-08, which is the same answer a poll that
//! saw no edit gives, so the refusal reached the terminal and no surface in the
//! instrument at all
...
///
/// Hashes and not text, and put in the store on this side of the channel, which
/// is the worker's thread. A session that records what it played has to name
/// the procedure, and the two ways to get the bytes there both end on the
...
pub struct Built {
    ...
    /// so it names the slot as well and no two builds in a run share one.
    pub id: u64,
    /// Every node of the rebuilt slot, each with the address a `procedure` record
    /// names it by — `(layer, index)`, the layer spelled the way the record spells
    /// it.
    ///
    /// The whole stack, not the one file that changed. A rebuild restates the slot,
    /// so a record of what played has to name every node in it; and it is a list
    /// rather than an L1 and some renderers because a slot can hold two geometries,
    /// a chain and a camera, all of which are procedures somebody may have just
    /// edited.
...
///
/// It is [`Watch::new`]'s argument list less the slot, plus the Set the slot is
/// running — which arrives at construction through [`Watch::snapshotting_to`]
/// rather than through `new`, because it is the history's and only the
/// history's. Every other field carries its argument's reason and is read
/// there, because they are the same fields and a second copy of them would be
/// as many places to get one of them wrong. Anything left out comes back as the
/// outgoing slot's, and the symptoms are the ones those fields are documented
/// against: a fold that silently un-selects, a camera that reverts to
/// `Orbit::default()`, salts that repaint every element. Worse, none of them
/// shows on the *load* — the aim states them, so the load is right and the
/// first later save is what goes wrong.
///
/// The slot is not here. A watcher that could be re-pointed at a different slot
/// would rebuild somebody else's deck, and the one thing enforcing *this
...
/// with, which is none, and the field is here rather than assumed so the sender
...
pub struct Aim {
    ...
    /// The Set this slot is now running, which is what every version it writes from
    /// here on is filed under — or `None` where it is running material no Set
    /// names, which is a pair somebody typed.
    ///
    /// It is here because a library load is the thing that moves it. A load
    /// re-points a watcher at the files of a different Set, and the id is as much a
    /// part of *what this slot is now* as the files are: a re-point that left it
    /// behind would file every later version under the Set before the load,
    /// silently and in a name nothing reads back
    /// ([`crate::history::Snapshots::record`] carries that argument, and `ADR-0276`
    /// is the record). It is one field of this rather than a value a surface keeps
    /// beside its deck, so that the answer to *what is this slot running* moves
    /// with the re-point that changes it and is restated by every re-aim, like
...
pub struct Watch {
    /// How many builds this watcher has requested, which with the slot makes an id
    /// no other build in the run shares.
    builds: u64,
    /// Where to put what was built, and who to tell. `None` and nothing here reads
    /// or writes a store at all.
    ///
    /// Wired whenever the run is editable, not only when a session is being
    /// recorded. The two were the same switch because the session stream was the
    /// only reader: [`Built::nodes`] existed to become `procedure` records, so
    /// there was no reason to store a build nobody would name. A live save is the
    /// second reader and it wants exactly the same fact — *which sources landed* —
    /// and it wants it in the ordinary case, which is `--watch` with no recording
    /// at all. Coupling them meant the one control that saves what is on screen
    /// worked only for a run that was already recording itself.
    ///
    ...
    /// Which slot this rebuilds. Carried only so that the diagnostics this prints —
    /// from a worker thread, interleaved with every other slot's — say which of the
    /// four they are about.
    slot: usize,
    /// The file the slot was spelled with. Not necessarily its L1 — what layer it
    /// is on is what its own `kind` declares, which the sort reads like it reads
    /// every other file's. It is first here only because it is first on the command
    /// line, and list order is chain order.
    head: crate::compile::Named,
    /// The rest of the slot's files, in the order they were spelled. Watched
    /// together with the head: a rebuild restates the whole stack, so an edit to
    /// any one of them recompiles all of them and the Set that lands is the one the
    /// files say.
    ...
    ///
    /// And rather than re-derived from `--merge`, which is the sharper half now
    /// that a Set file records a layering: a slot filled by `--load-set` from a
    /// composited file is compositing without the flag ever having been typed, so a
    /// rebuild that asked the flag would drop the slot to overdraw on the first
    /// save of any `.kir` in it — the L5 gone, every renderer back over one
    /// attachment, and any selection with it. That is `Watch::camera`'s failure in
    /// a louder register: the camera came back wrong, this comes back as a
    /// different picture entirely. `main::layering_for` is the one reading, taken
    /// once and handed to both the Set and this.
    layering: karakuri_engine::set::Layering,
    /// Which renderer the slot is folded to, restated on every rebuild beside the
    /// layering that makes it mean anything, and `None` for every input live.
    ///
    ...
    /// file that recorded a selection, which is the same discard the layering above
    /// describes and is invisible in exactly the same way: the picture changes and
    /// nothing says why.
    ///
    /// The run's own selection is not tracked here, and that is a limit rather than
    /// an oversight: `r` moves the fold at a frame this watcher never sees, so a
    /// rebuild restates what the *slot was loaded with*, and the reason is a
    /// request's: it has to be reproducible from a record stream, and what a hand
    /// did later is in the stream as a `select`.
    ///
    /// The params used to be the example of this and are no longer: a value a hand
    /// moved is carried across the swap by the Set itself
    ...
    live: Option<u32>,
    /// `--capacity` when it was given, and otherwise `None` — each geometry then
    /// runs at the default its own `capacity` declaration names.
    ///
    /// Not one resolved number. A rebuild recompiles the files, so a procedure's
    /// declared default is the *new* file's, and a slot with two geometries has two
    /// of them. Resolving at startup and carrying the answer would have pinned
    /// every later build to whatever the first one declared.
    capacity: Option<u32>,
    seed_salt: u32,
    /// One hash salt per geometry, restated on every rebuild rather than left to be
    /// derived there — for the reason `Request::bindings` gives, and with a louder
    /// symptom: a slot filled from a Set file is running at the salts that file
    /// recorded, so a rebuild that derived its own would repaint every element in
    /// it on the next save of a `.kir`.
    ///
    ...
    /// file's declaration, so resolving it at startup would pin every later build
    /// to the first one's; a salt depends on nothing in the file at all, so the
    /// run's own numbers are the answer for as long as the run lasts.
    ///
    /// A rebuild whose files declare *more* geometries than the run started with
    /// gets a salt for the ones this knows and a derived one for the rest, which is
    /// the rule `Set::build_many` follows for a short list — the ones that were
    /// already there keep their colours, and the new one is salted like a source
    /// nobody recorded, because that is what it is.
    salts: Vec<u32>,
    /// The six numbers the slot's camera is aimed with, restated on every rebuild
    /// rather than left to be taken from the outgoing Set — for the reason
    /// `Request::bindings` gives, and with the symptom that outlives the run:
    /// `Set::build_many` starts every Set from `Orbit::default()`, so a slot filled
    /// by `--load-set` used to lose the camera its file recorded on the first save
    /// of any `.kir`, and the next live save wrote the defaults into a new preset.
    /// The picture came back; the file did not.
    ///
    /// One resolved `Orbit`, on `salts`' side of the question above rather than
    /// `capacity`'s. A capacity is `Option` because the *new* file may declare one,
    /// so there is an answer only the rebuild can know; nothing in a `.kir`
    /// declares the built-in orbit's six numbers, so the run's own are the answer
    /// for as long as the run lasts and resolving them once at startup pins
    /// nothing.
    ///
    /// And not an `Option<Orbit>` either, which is where this parts company with
    /// both of them: a Set holds a built-in camera whatever its files declare and
    /// it is built at exactly this default, so a slot that loaded no `camera`
    /// record carries `Orbit::default()` and states it. "Nothing loaded" and "the
    /// default loaded" are the same Set, and an `Option` here would be a
    /// distinction nothing downstream could act on.
    camera: karakuri_engine::camera::Orbit,
    /// The values this slot was aimed with, stated on the build the aim causes and
    /// on no other — see [`Watch::stated`] beside it.
    ///
    /// It used to be restated on every rebuild, on the terms the salts, the camera
    /// and the bindings still are, and that is what made a knob unturnable: a slot
    /// pointed at a Set file carries *every declaration of every node* here,
    /// because a live save writes them all, so restating them put every parameter
    /// back to the file on the next save of any `.kir` and there was nothing an
    /// operator could do to a live Set that survived it. What replaces the
    /// restatement is [`karakuri_engine::Set::carry_moved_from`]: the values
    /// somebody stated travel across the swap on the Set itself, which is where
    /// they already were.
    ///
    /// The aim's own build still states them, and has to. At that moment nothing
    /// has applied them to anything — the aim is a description of a Set that is not
    /// built yet — so there is no live Set to inherit them from, and the same
    /// clause is what makes a *load* beat a ride: a value this build states is this
    /// build's answer for that key. See `carry_moved_from`.
    overrides: Vec<karakuri_engine::ParamWrite>,
    /// Whether the overrides above have been applied to the Set this slot is
    /// running. `true` from [`Watch::new`], because the caller built that Set with
    /// them and handed it over live; `false` from [`Watch::repointed`], because an
    /// aim describes a Set nothing has built yet.
    ///
    /// A `bool` and not a count: what it decides is whether *this* build is the one
    /// that has to state the values, and there is exactly one such build per aim.
    stated: bool,
    /// The interface, restated on every rebuild for the same reason the bindings
    /// are — and the more urgent one: a `control:` binding whose control did not
    /// survive the swap holds its param where it found it.
    published: Vec<karakuri_engine::set::Published>,
    /// Restated on every rebuild rather than read off the outgoing Set, for the
    /// reason `Request::bindings` gives: a request that depended on what happened
    /// to be live would not be reproducible from a record stream.
    bindings: Vec<Binding>,
    /// Which node fills each declared input slot, restated on every rebuild for the
    /// reason the bindings are — and with the loudest symptom of any of them: a
    /// slot nothing binds is refused outright, so a rebuild that left these out
    ...
    /// reads as "unchanged" rather than as a change every interval — a deleted file
    /// is not an edit to react to.
    stamps: Vec<Option<u64>>,
    /// A change has been seen but not yet acted on — see "Debouncing" above.
    settling: bool,
    /// Where a re-point arrives, and `None` for a watcher nobody can re-point —
    /// every offscreen path.
    ///
    /// `karakuri-cli`'s watched slots used to be in that list and are not any more.
    /// They were, for as long as nothing on that surface could change what a slot
    /// is wired with; `wire_input` can, and an edge is not a file, so the only way
    /// it reaches a build worker is an aim — see `main::Aiming`. A slot that
    /// surface does *not* watch still has no watcher at all, which is a different
    /// thing from a watcher that cannot be re-pointed.
    ///
    /// A channel rather than a shared cell for the reason this module polls rather
    /// than taking `notify`: the worker is a poll loop already, so there is nothing
    /// to wake and nothing to lock. It is also what makes a re-point *safe* to
    /// offer at all — the render thread hands over a description and touches
...
impl Watch {
    ...
    ///
    /// `set` is the Set this slot is running when the watcher is built, or `None`
    /// where it is running material no Set names. It is taken here rather than in
    /// [`Watch::new`] because it is the history's and only the history's — see
    /// [`Watch::snapshots`]. A later library load moves it, on the aim that
    /// re-points this watcher ([`Aim::set`]), so this is the first answer rather
    /// than the only one.
...
impl Watch {
    /// Take the newest aim off the channel, if one is waiting, and answer whether
    /// this watcher is now pointed somewhere else.
    ///
    /// The newest and not the next: the channel is drained to its end and only the
    /// last aim is taken. Two loads pressed inside one poll interval are one load
    /// as far as the picture is concerned — building the first of them would put a
    /// Set on screen that the operator has already replaced, and it would cost a
    /// compile to do it. That is the debounce's own reasoning with the roles
    /// swapped: there, a run of writes is one edit; here, a run of presses is one
    /// destination.
    ///
    /// `None` on the channel is a watcher nobody can re-point, which is every
    /// offscreen path and `karakuri-cli`'s own slots; a *closed* channel is a host
    /// that has gone, and reads exactly like nothing waiting.
    ...
    /// Compile what this watcher is pointed at and ask for it, or `None` where the
    /// files would not read, would not check, or would not assemble.
    ///
    /// Split out of [`Source::poll`] when a re-point became the second way in: a
    /// save reaches it through the debounce and an aim reaches it at once, and
    /// everything after that decision is the same work. Two copies of it would be
    /// two answers to *what does a rebuild restate*, which is the question every
    /// field on this struct is documented against.
