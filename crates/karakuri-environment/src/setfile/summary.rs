...
/// What one node of a Set is called — the one answer, for every surface that
/// has to name one.
///
...
/// fact. A card says what a procedure calls *itself* and says the same thing in
/// every Set that references the artifact; a `slot` name is what *this* Set
...
/// What one saved Set holds, read off the store: the id that names it, when its
/// file was written, and a line per node.
///
/// One value, rendered twice. The MCP `list_sets` tool and `--list-sets` answer
/// one question for two readers, and a library summarised once per reader is
/// two answers that agree until somebody edits one of them. Both render this;
/// neither opens a Set file of its own.
///
/// What it deliberately does not carry: what any of those nodes declares, what
/// this Set has turned anything to, and whether it builds. Those need a card
/// per artifact, the rest of the file, and a compile pass respectively —
...
///
/// The card reads are memoised across the whole listing, keyed by the artifact
/// rather than by the node: a library where fifty Sets reference one unnamed
/// geometry reads that card once. A named node reads no card at all, because
/// [`node_called`] would not have used it.
///
...
///
/// The name is the part no hash could carry, which is why this is not the
/// surface's `Nodes`: a name belongs to the *use* rather than to the procedure,
/// so it comes from the command line and travels beside the address rather than
/// inside it.
pub struct SavedNode {
    ...
    ///
    /// Which is the whole of what "lazily" means. The watcher puts what it builds
    /// in the store as it builds it, so a rebuilt node's bytes are already there
    /// and there is nothing to carry. A node still on its launch version has bytes
    /// that live only in this process — see [`crate::compile::Placed`] — so they
    /// ride along and reach the store at the moment a file names them. That is why
    /// a windowed run creates nothing until somebody presses `k`.
    ///
    /// These are the *compiled* bytes and never a re-read of the path, so a `.kir`
    /// rewritten since launch cannot reach a saved file.
    pub source: Option<Arc<str>>,
    /// The card that goes down with those bytes, carried on exactly the same
    /// condition and for the same reason — see [`crate::compile::Placed::meta`].
    /// `None` wherever `source` is `None`: a rebuilt node's card was written by the
    /// build that stored it, and there is nothing here to add.
    ///
    /// That is held by construction rather than asserted. The surface's
    /// `live_sources` asks the one predicate once and takes the pair off it,
    /// because it used to ask it twice — two derivations of "are these the bytes on
    /// screen" with nothing keeping them equal, where [`Sources::into_nodes`]
    /// writes the card *inside* the branch that puts the source and would have
    /// dropped a card whose `source` had gone `None` without a word.
    ///
    ...
    /// [`crate::compile::Placed::meta`], which declined the same thing on the same
    /// grounds.
...
/// build had landed, and the startup *paths* where none had — and the second
/// arm was a save that read the disk. Reading the disk answers a different
/// question: a slot rolled back to what it launched with, an edit that never
/// compiled, and a run with no watcher at all are all states where the file and
/// the picture disagree, and every one of them wrote down a version nobody had
/// seen. The hashes are seeded at launch instead — see the surface's
/// `Running::at_launch` — so there is one representation of "what bytes is this
/// node running", derived once from the text the compile read and a hash from
/// the first frame onward.
///
/// See `Live::save_set`. Owned, because it crosses onto the thread that does
/// the store I/O.
...
impl Sources {
    ...
    ///
    /// Nothing here reads a `.kir`, which is what collapsing the two arms bought: a
    /// node a build put there was stored by the watcher that built it, and a node
    /// still on its launch version carries the bytes the compile read. Either way
    /// the address was derived from bytes this process has held all along, and the
    /// only thing left to do is make sure the store has them — `put_artifact` is
    /// content-addressed, so putting one that is already there costs an `exists`
    /// and writes nothing.
    ///
    /// This is the moment a windowed run first touches the store. Seeding it at
    /// launch instead created a directory for every run whether or not anything was
    /// ever saved; see the surface's `Running::at_launch`.
