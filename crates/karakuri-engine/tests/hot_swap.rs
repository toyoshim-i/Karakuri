...
mod gpu {
    ...
    use karakuri_engine::governor::Estimated;
    use karakuri_engine::swap::{Event, HotSwap, Polled, Refusal, Request, Source};
    use karakuri_engine::{Basis, Binding, Curve, Gpu, Present, Set, Signals, Unfit, VideoSource};
...
"#;
    ...
    /// What `governor::budgeted` spends for this slot, read off the slot's own
    /// two readings.
    ///
    /// A candidate arrives with a measurement and an `estimate` since ADR-0356,
    /// and one rule decides which of them is spent: the estimate where it
    /// answers, the measurement where it refuses. Which one that is on any
    /// given adapter is the adapter's business — this machine's probe demotes
    /// to a host clock and two small rungs can come out with a negative slope —
    /// so the tests below assert the verdict against *this*, which is the rule,
    /// rather than against a basis chosen in advance, which would be a test of
    /// the machine.
    fn budgeted_by_the_one_rule(swap: &HotSwap) -> (Basis, Option<f32>) {
