...
fn try_build(gpu: &Gpu, l1: &str, l4s: &[&str]) -> Result<Set, SetError> {
    ...
    Set::build_many(&gpu.device, &gpu.queue, &compile(l1), &[], None, &refs, karakuri_engine::set::Layering::Overdraw, 2, 3)
}

fn build_over(gpu: &Gpu, l1: &str, l4s: &[&str]) -> Set {
    let compiled: Vec<Checked> = l4s.iter().map(|s| compile(s)).collect();
    let refs: Vec<&Checked> = compiled.iter().collect();
    let mut set = Set::build_many(&gpu.device, &gpu.queue, &compile(l1), &[], None, &refs, karakuri_engine::set::Layering::Overdraw, 2, 3)
