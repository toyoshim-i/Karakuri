...
impl Meters {
    ...
    /// arrives next was measured on an image that is no longer what the slot is
    /// showing, and a stale number presented as a live one is worse than no
    /// number. The buffers those measurements are travelling in come back to
