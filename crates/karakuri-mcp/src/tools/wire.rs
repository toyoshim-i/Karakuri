...
/// which geometry a morph blends towards is the exact failure this record
/// exists to end.* [`NodeAddress`]'s own documentation states the other half:
/// *so `Operation::WireInput` takes names and everything else takes this, and
/// the two are not interchangeable.* A tool here that took `{layer, index}`
/// because its five neighbours do would be spelling an edge in the one address
/// an edge may not be spelled in, and
/// `docs/principles/0086-a-procedure-knows-only-what-it-declares.md` is what it
/// would be breaking: the names are the *Set's* answer, and a procedure never
/// knows them.
///
/// Nothing here checks that the names resolve, and that is not laziness. The
/// nodes of a Set are named by the Set — `--set morph=warp.kir` names one, and
/// a bare file is named after the procedure inside it — and [`Slots`] holds
...
/// called whatever `--set` or a `proc` line called it and this server is not
/// the thing that decides what that may be.
...
///
/// It is `save_set`'s shape and `write_procedure`'s promise. The request goes
/// to the loop like a save, because only the loop holds what a slot is wired
/// with; but what comes back is a write's answer rather than a save's — an edge
/// lands the way an edited procedure lands, at a frame boundary and under the
/// same budget, so the tool answers when the edge is *written* and points at
...
/// thing a model can do here that a replay does not reconstruct — see the
/// module documentation, which says what would close it.
