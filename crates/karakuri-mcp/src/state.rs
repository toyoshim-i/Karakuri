...
///
/// A channel and not a shared lock: the frame path may wait for nothing, and a
/// swap is rare enough that the send costs less than the `eprintln!` beside it
/// already does.
pub enum Event {
    /// A build landed, was overloaded, or failed, in the words the operator saw on
    /// the terminal.
...
pub struct SaveRequest {
    /// Which deck slot. Checked against [`Slots`] before it is sent, so a slot this
    /// deck does not hold meets the refusal `read_procedure` gives it; the loop
    /// checks the range again, because it is the only thing that knows how many
    /// slots the *deck* has.
    pub slot: usize,
    /// What to file it under, or `None` to let the loop name it after the moment —
    /// which is what a key press gets, for the reason
...
impl Reply {
    ...
    /// record is written on — so what a model is told and what the stream says come
    /// from one place. `Err` is a refusal or a disk that said no, and both reach
    /// the client as a failed tool call.
    pub fn settled(self, said: Result<String, String>) {
        let _ = self.0.send(News::Settled(said));
    }
}

/// The other half of [`karakuri_environment::accepted_save`]'s call, so that
/// crate can hand a save's acceptance back to whichever client is waiting on it
/// without naming `Reply` — the dependency runs one way, from this crate into
/// `karakuri-environment`, and this `impl` is what lets it stay that way.
...
/// `docs/principles/0086-a-procedure-knows-only-what-it-declares.md` warns
/// about from the other end: *every surface that rebuilds one has to carry the
/// names it was spelled with rather than regenerate them.*
///
/// What the loop owes a request it takes, so that the contract is written where
...
pub struct WireRequest {
    ...
    ///
    /// The deck is on the operation — `Operation::WireInput` carries one — so an
    /// edge asked for here is a statement about *this* deck's Set and not about the
    /// run. `--edge` is run-wide because a launch flag is one command line for
    /// every Set it starts; a request made during a show names the deck it is
    /// about.
    pub slot: usize,
    /// The edge itself, in the engine's own type: both ends by name, which is
    /// `Record::Edge`'s decision and the reason `Operation::WireInput` is the one
    /// operation addressed by name at both ends.
...
pub struct Reporter {
    ...
    /// What clients have asked the loop to do. The receiving half, because this is
    /// the direction [`Event`] does not go in.
    pub(crate) requests: mpsc::Receiver<SaveRequest>,
    /// And the edges they have asked it to write. A channel of its own rather than
    /// one queue of a request enum: the two are drained by one loop but they are
    /// not one queue's worth of pressure — a deck being saved to a slow disk must
    /// not be able to fill the queue an edit is rewired through, and [`ASKED`] is a
    /// bound on each kind rather than on both together.
    pub(crate) wires: mpsc::Receiver<WireRequest>,
    /// And the operations they have asked it to perform, on a third channel for the
    /// second one's reason: a save to a slow disk must not be able to fill the
    /// queue a fader moves through, and [`ASKED`] is a bound on each kind rather
    /// than on all three together.
    pub(crate) operations: mpsc::Receiver<OperateRequest>,
    /// Reports the queue had no room for. Counted rather than lost quietly: a
    /// client that is told what happened must be told when it is not the whole
    /// story.
...
impl Reporter {
    ...
    ///
    /// Never blocks and never waits for room. A bounded queue and `try_send`: a
    /// client that has stopped asking must not be able to make the render thread
    /// wait, and the queue in front of the bounded history used to be unbounded, so
    /// it was not a report on the present at all.
    ...
    /// request ends in the same method the `k` key ends in. Empty on almost every
    /// frame, and an empty `collect` allocates nothing.
    pub fn saves(&self) -> impl Iterator<Item = SaveRequest> + '_ {
        self.requests.try_iter()
    }

    /// Every edge a client has asked to be written since this was last called.
    ///
    /// Drained beside [`Reporter::saves`] and on the same terms, and what the loop
    /// owes each one is written at [`WireRequest`].
    ///
    /// A loop that never calls this is not silently obeyed. Every request carries a
    /// [`Reply`] the client waits [`WIRE_REPLY`] for, so a run whose loop does not
    /// drain this answers *the render loop had not taken this edge* — which is
    /// true, is loud, and is the third of `docs/principles/0079-…`'s answers rather
    /// than a tool that reports work nobody did.
    ...
    /// called.
    ///
    /// Drained beside [`Reporter::saves`] and [`Reporter::wires`] and on the same
    /// terms, and what the loop owes each one is written at [`OperateRequest`].
    /// Already audited: `karakuri_operation::gate` ran on the connection thread, so
    /// a loop draining this performs what it is handed and does not judge it again
    /// — the audit is the one call the server makes and not a check every drain
    /// repeats.
    ///
    ...
    /// [`Reply`] the client waits [`OPERATE_REPLY`] for, so a run whose loop does
    /// not drain this answers *the render loop had not taken this operation*.
...
/// path.
///
/// Paths never cross the protocol. A client may be on another machine through
/// an `ssh -L`, where a path means nothing — and a tool that took one would be
/// inviting a model to write anywhere on the render machine's disk.
///
/// It is a handle and not a list, for [`crate::Opening`]'s reason. This used to
/// be the launch working copies, copied into [`serve`] before the window opened
/// and never written again — and a surface that loads a Set into a running slot
/// re-points that slot at *new* scratch files. So after one library load every
/// address this resolved was the pre-load layout: a `read_procedure` handed
/// back the source of a procedure the deck had stopped running, a
/// `write_procedure` wrote a file no watcher was looking at and reported that
/// it was being built, and a node the loaded Set does hold was refused for not
/// existing. Each of those is a plausible wrong answer on the one surface whose
/// reader is a program in a loop
...
/// arriving mid-load sees the layout before it or the layout after it and never
/// half of either.
...
impl Slots {
    ...
    /// A slot's files, each under the layer and the index the rest of this program
    /// addresses it by.
    ///
    /// The layer is read off the file, not off its position — the head included.
    /// Every path, the first one as much as the rest, is on the layer its own
    /// `kind` line names, at its position *within that layer*, keeping file order.
    /// It is [`crate::history::declared_kind`] here rather than a second scanner:
    /// two readers of a `kind` line would be two answers to what layer a file is
    /// on, and the layer a version is filed under has to be the layer an agent
    /// addresses it by.
    ///
    /// The head used to be filed as `L1` whatever it declared, and nothing
    /// downstream agreed with that: [`crate::compile::sort_compiled`] matches on
    /// the head's own `kind` like every other entry, and `history::seed` reads the
    /// head's `kind` line too. `--set a,b,c` takes whatever the operator typed
    /// first, so a slot headed by a `kind L3` was addressed as `L1:0` — a
    /// `write_procedure` carrying a valid L1 passed the kind guard, overwrote the
    /// camera, and left the slot with a second geometry, no camera and no way to
    /// reach the real L3 at all.
    ///
    /// A text scan and not a parse, for that function's reason: this surface works
    /// without compiling anything, and a file that does not compile must still be
    /// addressable — it is the one a model most needs to read. One that cannot be
    /// read, or that declares no `kind`, is counted as a renderer, which is what
    /// `history::seed` makes of it and what the compile is about to refuse it as.
    ///
    /// Read on every call rather than worked out once at startup. The files are a
    /// handful and nothing here is on a frame path, and a layout cached beside a
    /// directory `--watch` is editing is a layout that can be wrong.
    ///
    /// The paths come back owned, because what they are read out of is a lock this
    /// must not hold past the call — see [`Slots::held`].
    pub(crate) fn nodes(
        &self,
        slot: usize,
    ...
    /// Whether this deck holds `slot` at all, without reading a byte off disk.
    ///
    /// [`Slots::nodes`] answers this too, and costs a `read` per file of the slot
    /// to do it, because it works each node's layer out of the file's own `kind`
    /// line. `save_set` wants nothing but the range and was calling `nodes` for it
    /// — under the state mutex, which is the one lock in this server a slow disk
    /// can be held across, and it is held across every other connection's request
    /// as well. The number was [`Slots::count`] all along.
    ...
    ///
    /// It exists for the panel's landing on a row of the edit history, and it
    /// delegates rather than repeating: turning an address into a file is this
    /// type's own walk over a slot's `kind` lines, and a second one beside it would
    /// be two answers to *which file is `L4:0` of this slot* — the mistake
    /// [`Slots::nodes`]' own head records under a different name. The answer
    /// follows a library load because this handle does — see [`Slots::re_point`],
    /// which is the same reason the server's own address resolution follows one.
    ///
    /// A layer word this crate does not write is an `Err` naming it, on the same
    /// terms an address the slot does not hold is.
    pub fn file(
        &self,
        slot: usize,
        layer: &str,
...
///
/// One list, so the enum a client is handed, the address this resolves and the
/// `kind` a written source must declare cannot disagree. They did: the schema
/// offered `L1` and `L4` alone for as long as a slot could hold five kinds of
/// node, so the most interesting material in the language — the deformations,
/// the camera, the field — was in the files and unreachable from the one
/// surface built for editing them.
///
...
/// `docs/contributing.md` §4 (a name means one thing across the system) is the
/// question to answer.
...
/// it cannot serve calls `handle_alloc_error`, which aborts the process. It is
/// not catchable and it is not confined to this thread — a fifty-six byte
...
/// of everything in front of the bounded part: two hundred thousand events,
/// each holding a `String` allocated on the render thread, materialised in one
/// drain. Dropped rather than queued past this, and counted, because a report
/// with a hole in it must say so.
pub(crate) const QUEUED: usize = 256;

/// How many requests of one kind may be waiting for the render loop at once.
///
/// Per channel, not per loop: the saves and the edges are counted apart — see
/// [`Reporter::wires`] — so a client waiting on a slow disk cannot use up the
/// room an edit needs to be rewired through.
///
...
///
/// The same rule and not a second one: `checked_id` is still what refuses a bad
/// id, because a client may send anything whatever a schema says, and this is
/// that rule written where a model reads it *before* calling. A charset a tool
/// enforces and does not publish is a refusal a caller had no way to avoid —
/// which is [`kept`]'s argument about `"id": null`, applied to the argument it
/// is about rather than to a null.
///
/// The one constraint that cannot be published this way is the deck's: how many
/// slots this run holds is not knowable when `tools/list` is answered, and
...
/// waits for a *disk*, whose worst case is somebody else's mount; this waits
/// for the loop to reach the top of a frame, apply the edge and say so — which
/// is one frame at any frame rate anybody plays at, and this many seconds is
/// room for a loop that is busy rather than an estimate of the work.
///
...
pub(crate) struct State {
    /// Where each slot's procedures are *now*, read through on every call and never
    /// copied out — see [`Slots`], whose head is the whole of why this is a handle.
    /// A layout this server held would be the launch one forever, and a library
    /// load re-points a slot at new scratch files.
    pub(crate) slots: Slots,
    /// Where the library lives — `--store DIR`, the same root every other half of
    /// this run reads and writes. A root rather than an open [`Store`]; see
    /// [`serve`].
    pub(crate) store: std::path::PathBuf,
    /// Whether `--watch` is on. Without it a written procedure sits on disk and
    /// changes nothing, which a model has no way to discover and every reason to be
    /// told.
    pub(crate) watching: bool,
    /// Which classes the operator has opened, read on every call — see
    /// [`crate::Opening`] and [`audited`]. A parameter of [`serve`] and not a value
    /// this module chooses: a server that decided its own opening would be the
    /// surface holding the authority, which is the shape
    /// `docs/principles/0090-a-surface-offers-it-never-decides.md` rules out. Nor
    ...
    /// that has stopped taking requests must produce an answer rather than a thread
    /// that never returns.
    pub(crate) asked: mpsc::SyncSender<SaveRequest>,
    /// And where an edge a client asks for goes, on the same terms and for the same
    /// reasons — see [`WireRequest`] and [`Reporter::wires`].
    pub(crate) wiring: mpsc::SyncSender<WireRequest>,
    /// And where an operation a client asks for goes, on the same terms again — see
    /// [`OperateRequest`] and [`Reporter::operations`]. The audit has already run
    /// when something is put here: [`audited`] is between [`asked`] and
    /// [`perform`], and nothing else constructs an [`OperateRequest`].
