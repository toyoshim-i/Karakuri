...
//! carries no data. Serialising it back out would not reproduce the line
//! that produced it; it would produce `{"t":"unknown"}`, discarding whatever
//! the original line actually said. That is worse than dropping the line
//! outright, because it looks like fidelity while corrupting the record.
//!
//! [`Line`] fixes this without touching `record.rs`: it carries the exact
//! source text alongside the parsed [`Record`], and writing always emits
//! that text back verbatim. A record built in memory (never read from a
//! file) has no "original" text, so [`Line::new`] serialises it once, at
//! construction, and that becomes its text from then on.
...
/// One line of an ndjson file: the record it parses to, plus the text it
/// came from.
...
impl Line {
    /// Wrap a record constructed in memory. Serialises it immediately to
    /// fix its on-disk text.
    ///
    /// Do not use this for [`Record::Unknown`] — that variant only ever
    /// arises from parsing a line the store does not understand, and
    /// serialising it fresh would fabricate a `{"t":"unknown"}` line that
    /// never existed. Round trip unknown lines through [`read`] and
    /// [`write`] instead, which preserve their original text.
...
///
/// A blank line has no record to parse and is reported as malformed, same
/// as any other unparsable line, carrying its 1-based line number.
...
/// crash mid-write never corrupts whatever was already there.
