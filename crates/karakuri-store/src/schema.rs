...
/// Detects the schema version of an ndjson stream (defaults to Version 1 if header is absent).
...
/// Ensures `Record::Header` with `CURRENT_SCHEMA_VERSION` is at line 0, preserving all records.
