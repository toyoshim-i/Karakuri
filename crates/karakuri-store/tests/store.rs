...
fn open_establishes_layout_and_is_idempotent() {
    ...
    // **The operator's own procedures**, established for `sandbox/`'s reason
    // and one more: the Library bay lists this directory on the first press of
    // a run, whether or not anything has ever kept a node into it (ADR-0338).
    assert!(root.join("procedures").is_dir());
}
