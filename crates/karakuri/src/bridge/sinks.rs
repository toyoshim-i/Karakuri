...
/// itself, the view [`Present::draw`] draws into, the registration `egui` reads
/// it by, and its size in physical pixels.
///
/// Two call sites on the day it is written, which is this repository's rule
/// about an abstraction and is the same standing
...
///
/// The split is where it is because of what a test can call. Deciding *which
/// rectangle, at what size* inside `window_event` is deciding it somewhere
/// `winit` will not let a test reach — and sizing deck A's texture from the
/// picture's rectangle was injected there once and every test still passed.
/// [`aims`] and [`Presented::aim`] are that decision, whole, outside the event
/// handler; `mod gpu` calls them the way the frame does.
///
/// Neither of these is presented anywhere, which is why [`Sink::present`] is
/// `Ok(())` for both: the picture and the preview cell are textures the panel
/// samples, and the thing that reaches a display is the window — which is not a
/// sink here. `karakuri-cli`'s window shows the canvas; this window shows the
/// panel, and the panel goes in `finally`.
pub(crate) struct Presented {
    /// The texture. Held because the views below are of it, and because freeing the
    /// `egui` registration does not free this — `egui-wgpu` stores a bind group for
    /// a registered native texture and no texture at all, so dropping this is the
    /// only thing that releases the memory. Read only by `mod gpu`, which asks it
    /// what size it came out, and that is the whole of why the lint has to be told:
    /// the window does not read it and the window is not what it is for.
    #[allow(dead_code)]
    ...
    ///
    /// It is a field rather than an argument because the two questions are asked at
    /// different moments and by different callers: the frame aims every sink before
    /// it composes, and `compose` then asks each one for itself. A `Presented` that
    /// has never been aimed answers no, which is the right answer — it has a 1x1
    /// placeholder texture and nothing has said where it goes.
    pub(crate) aimed: bool,
}

impl Presented {
    /// A sink aimed at the rectangle its region has, or at nothing where the region
    /// is folded away.
    ///
    /// It goes through [`Presented::aim`] rather than sizing the texture here, so
    /// that *which rectangle, at what size* has exactly one derivation in this file
    /// and the window's first frame cannot disagree with the window it opened at.
    /// The 1x1 below is never drawn into and never on screen: it is what `aim`
    /// replaces on the same statement.
    ///
    /// The `freed` it counts into is discarded, and that is the one place in this
    /// file where it may be. [`Engine::freed`] is a tally of registrations leaked
    /// by a resize, and this is construction — a caller that saw a 1 here would be
    /// told a leak had already happened before the window drew.
    pub(crate) fn new(
        gpu: &Gpu,
    ...
    /// The texture, the view the engine draws into, and the registration `egui`
    /// reads it by, at a size in physical pixels. One constructor because the three
    /// are made together and are replaced together, and private to this type
    /// because a caller aims at a rectangle — turning one into a size is
    /// [`Presented::aim`]'s and [`Presented::fit`]'s alone.
    pub(crate) fn made(
        gpu: &Gpu,
        renderer: &mut egui_wgpu::Renderer,
        label: &'static str,
    ...
    /// Where this sink goes this frame, and how big its texture therefore is — one
    /// statement, and it is the whole of what [`Sink::acquire`] then answers from.
    ///
    /// Returns what the console should draw, or `None` where there is no rectangle.
    /// The returned [`Picture`] carries the same rectangle the texture was just
    /// sized from and the id the sizing may have just replaced, which is the
    /// pairing `karakuri_console::view::Picture`'s own documentation asks for:
    /// whoever sized the texture and whoever placed it are one statement, so a
    /// texture sized from the window and drawn into the picture's region cannot be
    /// written by accident, and a resize cannot leave a freed id in the view.
    ///
    /// Called before [`compose`], and it has to be: `acquire` takes only a `&Gpu`,
    /// and this needs the rectangle, the scale factor and the renderer. It is also
    /// a reallocation on the frames a size changed, so it belongs at the top of the
    /// frame rather than mid-pass — see [`Presented::fit`].
    pub(crate) fn aim(
        &mut self,
        gpu: &Gpu,
        renderer: &mut egui_wgpu::Renderer,
    ...
    /// The region is a different size, so the texture is remade at that size and
    /// the registration it replaces is freed.
    ///
    /// Returns whether anything was remade, which is `false` on all but a handful
    /// of frames — every frame of a drag on the program's height is one of them,
    /// and that is exactly the case the free is for. A `register_native_texture`
    /// per dragged frame with no `free_texture` beside it is a bind group and a
    /// sampler leaked per frame, for as long as somebody keeps hold of a divider.
    ///
    /// A reallocation, so it is the frame's first act and not something done
    /// mid-pass
    /// ([P-0091](../../../docs/principles/0091-cost-is-known-before-it-is-paid.md)
    /// is about the render thread, and this is that thread; what it costs is paid
    /// on the frames a size changed and on no others).
    ///
    /// `freed` is handed in rather than kept here because the tally is the whole
    /// engine's — one number over both textures, which is what [`Engine::freed`] is
    /// and what `mod gpu` asserts on. Taking it as an argument is what makes it
    /// impossible for a call site to remake a texture and forget to count what it
    /// freed.
    pub(crate) fn fit(
        &mut self,
        gpu: &Gpu,
...
/// view first. `None` back means no output is enabled at all, and the caller
/// leaves the size where it is rather than picking one.
///
...
/// that needs a rule, and there are two candidates. A componentwise maximum is
/// refused: 1920x1080 beside 1024x1280 would give 1920x1280, which is a size no
/// output is and nobody asked for, and it upscales both of them in one axis
/// while claiming to prevent upscaling. Largest by pixel count wins, and it
/// hands back one output's own size — so the frame is always exactly right for
/// at least one destination and a downscale for the rest. The tie goes to the
/// earlier entry, which is the program view, because the picture is the output
/// an operator is looking at while they decide.
///
...
/// and re-deriving a size there would reallocate every slot target, the mix and
/// the master chain in order to change a picture nobody is looking at — and
/// reallocate them again on the way back. So the fallback to [`CANVAS`] is the
/// run's starting value rather than a re-derivation, and this function says
/// *nothing to say* instead of guessing.
///
/// The rejected reading is ADR-0247's own sentence taken literally — *"the
/// session's `canvas` record is what it falls back to when no output says
/// anything"* — which is right about a run that has never had an output and
/// wrong about one whose outputs are all momentarily off, and cannot tell the
/// two apart from inside this function. The starting value is where that
...
/// applied in the mix and a slot's target is upstream of the mix. That is what
...
///
/// Gated on `aimed` and on there being a slot to sample, and on nothing else.
/// Residency was the third gate and is not: a cell that goes dark when its deck
/// goes off air is dark at exactly the moment an operator is deciding whether
/// to bring it back. [`Engine::aim`] asks the same two questions, so a cell
/// cannot be aimed and not drawn.
///
...
/// the size of what it is drawn into rather than of the window. Which rectangle
/// each one gets is [`aims`]; this is the *at what size* half, and
