...
//!
//! It lives in the crate at the bottom of the dependency graph so that nothing
//! which needs it has to keep a copy. It was in `karakuri-codegen`, which the
//! estimator in [`crate::cost`] cannot call — `karakuri-codegen` depends on
//! this crate and not the reverse — so stage 4 carried its own copy, the two
//! drifted the moment the real one changed, and `drift_shell` was reported at
//! 192 bytes an element where its buffers are 104. Stage 4 has since stopped
//! asking the question at all, for a reason of its own that [`crate::cost`]
//! states, and the argument for this address survives it: what an element
//! struct contains is decided by a procedure's `emit`, by [`Attr`]'s own widths
//! and by which derivations are stored, and those are all facts of this crate.
//! A placement rule kept anywhere above them is a second description of them,
//! waiting to disagree.
//!
...
//! procedure emitting three attributes bound 12 storage buffers in its compute
//! stage (3 synthetic + 3 declared, times prev and next) — more than the WebGPU
//! default limit of 8, and twice the downlevel default of 4. All per-element
//! state that lives in the *same* buffer as everything else of its own
//! direction packs into one generated `Element` struct instead: one slot per
//! entry (`seed`, `birth_frac`, then `emit` in declaration order), each at its
//! own width and at the offset WGSL's layout rules give it. A `vec3` is 16-byte
//! aligned and 12 bytes long, so a scalar declared after one lands in the four
//! bytes it leaves: `position, size` is one 16-byte block rather than two.
//!
//! `alive` is the one exception: it leaves the struct entirely and becomes its
//! own tight `array<u32>` buffer, [`ALIVE_BYTES`] per element, because the
//! compaction scan (`crates/karakuri-engine/src/shaders/scan.wgsl`) reads alive
//! flags as a dense array with no stride arithmetic. Packing it into `Element`
//! would mean the scan needs to know the struct's per-procedure size just to
//! skip to the next flag; a dense array needs nothing but the element count.
//!
//! # Why a crate that emits no WGSL spells WGSL type names
//!
//! [`StorageElemTy::align`] and [`StorageElemTy::size`] are WGSL's table, not a
//! table this project chose — a crate that owns them already knows WGSL's type
//! system, and [`StorageElemTy::wgsl_name`] beside them is the same fact under
//! its own spelling. Splitting the spelling off into the crate that emits the
//! text would put one type's three properties in two places, which is the shape
//! of defect this module exists to end. `karakuri-ir` already hands out WGSL
//! identifiers besides: every [`Attr::name`] is one, and the generated struct's
//! field names are exactly those strings.

use crate::ast::{Attr, Derivation, Ty};

/// Bytes per element in the alive buffer.
///
/// A dense `array<u32>` and not a slot in `Element`, so this is a stride of its
/// own rather than an offset into one — see the module doc for why the
...
///
/// The one rounding in this project's layouts, shared by the element placement
/// here and by `karakuri-codegen`'s uniform-struct builder rather than written
/// twice — the two are the same rule out of the same specification, and a
/// private twin in each crate is how they would come to differ.
...
///
/// These are WGSL's numbers, not this crate's. A layout that invented its own
/// would be a second place for the same fact, and the two would disagree the
/// first time a field type was added: the host writes bytes at
...
impl StorageElemTy {
    /// Every variant, so that a lookup *by* WGSL spelling can be a search over this
    /// list instead of a second spelling table written backwards beside
    /// [`StorageElemTy::wgsl_name`].
    ///
    /// A variant missing from here fails loudly rather than quietly.
    /// [`StorageElemTy::from_wgsl_name`] answers `None` for it, and the caller that
    /// asks — `karakuri-codegen`'s uniform table — panics naming the type it could
    /// not place. That is the affordable failure: the unaffordable one is a
    /// spelling that resolves to the wrong width, because nothing downstream of a
    /// wrong width is a compile error.
...
///
/// This was every field padded to a `vec4`, so that the stride was `(2 +
/// emit.len()) * 16` with no per-attribute case analysis. The case analysis is
/// four lines of [`StorageElemTy`] and the padding was 39% of every element
/// buffer across the geometries this repository ships — and an amplifying stage
/// multiplies exactly that number, so the same fraction comes off the largest
...
pub struct ElementSlot {
    /// The struct field name, and the WGSL identifier used everywhere the slot is
    /// read or written (`prev[i].<name>`, `next[i].<name>`,
    /// `elements[elem].<name>`). Every possible value — `seed`, `birth_frac`, and
    /// every [`Attr`] name — is already a valid WGSL identifier and never a
    /// reserved word, so unlike a `param` this needs no mangling.
    pub name: &'static str,
    /// `None` for the two attributes no procedure can name: `seed` and the birth
    /// fraction. They are allocated unconditionally because identity and spawn
    /// timing need them whether or not the procedure's `emit` list does.
    pub attr: Option<Attr>,
    pub elem_ty: StorageElemTy,
    /// Byte offset of this field within one `Element` entry, by WGSL's own
    /// placement rules — published rather than left for a caller to recompute, same
    /// reasoning as `karakuri_codegen::layout::UniformField::offset`, and now
    /// load-bearing rather than merely convenient: it is no longer `index * 16` and
    /// there is no second way to arrive at it.
    pub offset: u32,
}

/// The ordered slots of a generated `Element` struct, plus the struct's byte
/// stride. This is the contract `karakuri-engine` drives: it sizes the element
/// buffer as `capacity * stride`, and [`ElementLayout::offset_of`] is how it
/// finds where a given slot's bytes land when it needs to write one directly
/// (initializing `seed` and `birth_frac` for a spawn-less procedure — see
/// `karakuri-engine`'s `node::Simulation::initialize`).
#[derive(Debug, Clone, PartialEq)]
pub struct ElementLayout {
    /// `seed`, then `birth_frac`, then the engine-written slots this position
    /// carries, then `emit` in declaration order — see [`generate_element_layout`].
    pub slots: Vec<ElementSlot>,
    /// The array stride: the struct's size rounded up to its own alignment, which
    /// is the rule WGSL applies to `array<Element>`. Not a function of the slot
    /// count.
    pub stride: u32,
    /// Attributes readable here that have no slot, synthesised at the read site
    /// from one that does.
    ///
    /// This is what makes an `ElementLayout` the *contract* rather than a byte
    /// layout: two of the things a consumer may name are not fields, and a reader
    /// that only had `slots` would have to be told about them separately — which is
    /// a second place for one fact. Everything that lowers an attribute read asks
    /// this list first.
...
/// element has.
///
/// Conditional, because each one costs its own width on every element of every
/// Set that has it — four bytes for `copy`, plus whatever alignment it drags
/// behind it. `seed` and `birth_frac` are unconditional because identity and
/// spawn timing are properties of an element as such; what is here is a
...
///
/// One struct rather than a `bool` parameter, and it now holds one flag. It was
/// built expecting company: `source`, from a Set with several L1 sources, was
/// going to be the second synthetic slot, and threading two independent
/// booleans through the same call sites is two places to forget one. `source`
/// did not arrive and will not — a chain is instantiated per source, so which
/// source an element came from is the same value for every element that
/// instance will ever touch, which makes it a uniform rather than four bytes
/// restating a constant on every element of every merged Set. See
/// `karakuri-engine`'s `Source`, and `docs/ir-spec.md` under `source`, where
/// that reversal is recorded.
///
/// The struct stays because the shape is still right: the next conditional slot
/// lands as a field here rather than as a second parameter at every call site.
/// It is a struct waiting to be needed, not one that already is.
...
///
/// Named here rather than beside the rule in [`Attr::derivation`], because a
/// name is a WGSL identifier that appears in a generated struct and at every
...
/// Builds the `Element` struct layout for a procedure's `emit` list: `seed`
/// first, then `birth_frac`, then whichever of [`Synthetic`] this position
/// carries, then `emit` in declaration order. Fixed order because it is the
/// shape of a struct in WGSL text, not a set of independently addressable
/// bindings — unlike the old per-attribute binding numbers, there is no freedom
/// to reorder without changing what every `prev[i].<field>` access compiles to.
///
...
mod tests {
    ...
    /// `copy` is allocated only where something upstream amplified, which is the
    /// whole reason [`Synthetic`] exists as a parameter rather than the slot being
    /// unconditional like the two above it — and it is now free besides, because
    /// `seed` and `birth_frac` leave eight bytes of the first block unused and
    /// `copy` lands in four of them.
    #[test]
    ...
    /// A slot is its attribute's own width, and the offsets are WGSL's. Every field
    /// used to be a padded `vec4` so that the stride was `slots.len() * 16` with no
    /// case analysis. The case analysis is four lines and the padding was a fifth
    /// of every element buffer in the system — and an amplifying stage multiplies
    /// exactly that number.
    #[test]
